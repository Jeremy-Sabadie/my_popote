...
export class RecipesComponent implements OnInit {
  ...
  /**
   * Recettes affichées après application de la recherche locale.
   *
   * Les tags restent filtrés par l'API.
   * La recherche texte est appliquée immédiatement côté front
   * sur le nom de la recette et les noms de ses ingrédients.
   */
  get filteredRecipes(): Recipe[] {
    const normalizedSearchTerm = this.normalizeSearchText(this.searchTerm);
    ...
    return this.recipes.filter((recipe) => {
      const recipeName = this.normalizeSearchText(recipe.name);

      const recipeMatches = recipeName.includes(normalizedSearchTerm);

      const ingredientMatches = recipe.ingredients.some((ingredient) =>
        this.normalizeSearchText(ingredient.ingredientName).includes(
   ...
   */
  quantityStep(unit: string | null | undefined): number {
   ...
   */
  normalizeIngredientQuantity(index: number): void {
    const ingredient = this.ingredients.at(index);

    const quantityControl = ingredient.get('quantity');
    ...
    quantityControl.setValue(this.normalizeQuantity(quantity, unit), {
      emitEvent: false,
    });
  }
  ...
  private buildRecipeRequest(): RecipeRequest {
    ...
    return {
      ...
      ingredients: value.ingredients.map((ingredient) => ({
        ...
        quantity: this.normalizeQuantity(ingredient.quantity, ingredient.unit),
    ...
    this.recipeService.deleteRecipe(recipe.id).subscribe({
      ...
      error: () => {
        void Swal.fire({
          ...
          text: ' car elle apparait dans une semaine déjà validée.',
          icon: 'error',
          confirmButtonText: 'Fermer',
        });
      },
  ...
  /**
   * Normalise le texte afin que la recherche soit
   * insensible à la casse et aux accents.
   *
   * Exemple :
   * "Purée" et "puree" correspondent.
   */
